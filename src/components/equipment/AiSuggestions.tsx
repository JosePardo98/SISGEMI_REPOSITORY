...
export const AiSuggestions: React.FC<AiSuggestionsProps> = ({ equipment, maintenanceHistory }) => {
  ...
  const fetchSuggestions = async () => {
    ...
    const historySummary = maintenanceHistory
      .slice(0, 5) // Limit to last 5 records for brevity
    ...
    try {
      ...
      setError('No se pudieron obtener las sugerencias de IA. Inténtelo de nuevo.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Card className="shadow-lg mt-8">
      <CardHeader>
        ...
        <CardDescription>
          Obtenga una lista de procedimientos de mantenimiento sugeridos por IA basados en el tipo de equipo, historial y puntos comunes de falla.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Button onClick={fetchSuggestions} disabled={loading} className="mb-4 bg-accent hover:bg-accent/90 text-accent-foreground">
          {loading ? (
            <>
              <Loader2 size={18} className="mr-2 animate-spin" /> Obteniendo...
            </>
          ) : (
